...
if (isset($_SESSION['role']) AND $_SESSION['role'] == 'admin' AND isset($_SESSION['mail'])):
    ...
    <div id="app" class="app app-header-fixed app-sidebar-fixed app-content-full-height">
        ...
        <div id="content" class="app-content">
            ...
            <!-- END breadcrumb -->
            <!-- BEGIN page-header -->
            <h1 class="page-header">Liste des devis<small></small></h1>
            <!-- END page-header -->
            <!-- BEGIN panel -->
            <div class="panel panel-inverse">
                <!-- BEGIN panel-heading -->
                <div class="panel-heading">
                    <h4 class="panel-title">Filtrer les devis</h4>
                </div>
                <!-- END panel-heading -->
                <!-- BEGIN panel-body -->
                <div class="panel-body">

                <table id="data-table-default" class="table table-striped table-bordered align-middle" name="table-devis">
                        <thead>
                        <tr>
                            <th class="text-nowrap">Devis N°</th>
                        ...
                        <?php foreach ($devis as $d) : ?>
                            ...
                            <tr class="odd gradeX">
                                <td width="1%" class="fw-bold text-white"><?= $d['cmd_devis_id'] ?></td>
                                <td><?= $d['cmd_devis_reference'] ?></td>
                                <td><a href="profile.php?id=<?= $d['users_id'] ?>"><?= $d['nom_client'] . ' ' . $d['client_prenom'] ?></a></td>
                                <td><?= $d['users_entreprise'] ?></td>
                                <!--<td><?= $d['cmd_devis_prixHT'] ?></td>-->
                                <td><?= $d['cmd_devis_prixventettc'] ?></td>
                                <!--<td><?= $d['nombre_produits'] ?></td>
                                <td><?= $d['quantite_totale_demandee'] ?></td>-->

                                <td><?= $d['cmd_devis_etat'] ?></td>

                                <!--<td><?= date('Y/m/d', $d['cmd_devis_datein']) ?></td>-->
                                <td><?= date('Y/m/d', $d['cmd_devis_dateout']) ?></td>
                                <!--<?php if ($d['cmd_devis_dateSouhait'] != 0) : ?>
                                    <td><?= date('Y/m/d', $d['cmd_devis_dateSouhait']) ?></td>
                                <?php else : ?>
                                    <td></td>
                                <?php endif; ?>-->
                                <td>
                                    <a href="fiche_devis.php?id=<?=$d['cmd_devis_id']?>" class="btn btn-sm btn-primary w-100px mb-2">Détails</a> <br>
                                    <a href="modif_commande_devis.php?id=<?=$d['cmd_devis_id']?>" class="btn btn-sm btn-warning w-100px mb-2">Modifier</a><br>
                                    <a href="#modal-alert" class="btn btn-sm btn-danger w-100px" data-bs-toggle="modal" data-id="<?= $d['cmd_devis_id'] ?>" onclick="setDeleteCommandId(this)">Supprimer</a>
                                </td>
                            </tr>
                    ...
                    <form method="POST" id="deleteForm">
                        <div class="modal-body">
                            <div class="alert alert-danger">
                                ...
                                <p>Êtes-vous sûr de vouloir supprimer ce devis ?</span></p>
                            </div>
                        </div>
