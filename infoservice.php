...
<head>
    <meta charset="utf-8" />
    <title>A2MI | Service Informatique</title>
...
if (isset($_SESSION['role']) AND $_SESSION['role'] == 'admin' AND isset($_SESSION['mail'])):
    ...
    <div id="app" class="app app-header-fixed app-sidebar-fixed app-content-full-height">
        ...
        <div id="content" class="app-content">
            ...
            <!-- END breadcrumb -->
            <!-- BEGIN page-header -->
            <h1 class="page-header">Tableaux des informations de la page Service Informatique<small></small></h1>
            <!-- END page-header -->
            <!-- BEGIN row -->
            <div class="row">
                <!-- BEGIN col-12 -->
                        ...
                        <div class="panel-body">
                            <table id="data-table-combine" class="table table-striped table-bordered align-middle">
                                ...
                                <tr>
                                    ...
                                    <th class="text-nowrap w-10px">Active</th>
                                    <th class="text-nowrap w-20px">Logo</th>
                                    <th class="text-nowrap w-50px">Titre</th>
                                    <th class="text-nowrap w-500px">Images</th>
                                    <th width="1%"></th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php foreach ($allinfoservices as $infoservice) : ?>
                                    <tr>
                                        <td><?= $infoservice->site_post_id ?></td>
                                        <td>
                                            <?php if ($infoservice->site_post_active >= 1) : ?>
                                        ...
                                        <td>
                                            <?php if (!empty($infoservice->site_post_photo)) : ?>
                                                <img src="img/logo/<?= $infoservice->site_post_photo ?>" class="rounded h-40px" />
                                            <?php else : ?>
                                                <img src="img/placeholder.jpg" class="rounded h-30px" />
                                            <?php endif; ?>
                                        </td>
                                        <td><?= $infoservice->site_post_title ?></td>
                                        <td>
                                            <?php
                                            $req = $pdo->prepare("SELECT * FROM site_img WHERE site_img_id_post = $infoservice->site_post_id");
                                            $req->execute();
                                            $allisimg = $req->fetchAll(PDO::FETCH_OBJ);
                                            ?>
                                            <?php foreach ($allisimg as $isimg) : ?>
                                                <a href="img/S-info/<?= $isimg->site_img_photo ?>" data-lightbox="<?= $isimg->site_img_id_post ?>">
                                                    <img src="img/S-info/<?= $isimg->site_img_photo ?>" class="rounded h-40px m-1" alt="<?= $isimg->site_img_title ?>" />
                                                </a>
                                            <?php endforeach; ?>
                                        ...
                                        <td nowrap>
                                            ...
                                            <a href="#" class="btn btn-sm btn-warning w-40px me-1"><i class="fas fa-lg fa-fw me-10px fa-images"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
